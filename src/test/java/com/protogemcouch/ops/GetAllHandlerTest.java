...
class GetAllHandlerTest {
    ...
    @Test
    void handle_opaque_geode_values_in_repository_result_are_encoded_in_response() {
        Repository repository = mock(Repository.class);
        ChannelHandlerContext ctx = mock(ChannelHandlerContext.class);
    ...
    private static byte[] geodeUuidValue() {
        return hexToBytes(
                "62123e4567e89b12d3a456426614174000"
        );
    }

    private static byte[] geodeBigIntegerValue() {
        return hexToBytes(
                "5f0d018ee90ff6c373e0ee4e3f0ad2"
        );
    }

    private static byte[] geodeBigDecimalValue() {
        return hexToBytes(
                "60090000000900000002"
        );
    }
