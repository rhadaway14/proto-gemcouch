...
/**
 * Dev tool: capture the GET_PDX_TYPE_BY_ID (92) request/response of a real Geode server. A separate
 * process ({@code PutOnce ... pdx}) must first write a PDX object DIRECTLY to the server so the type
 * is registered there; this tool then connects a <em>fresh-JVM</em> reader through a logging proxy and
 * {@code get(KEY)}s it with {@code pdx-read-serialized}, which forces the client to fetch the unknown
 * PdxType by id from the server. The dumped bytes are the response shape the shim must serve so a
 * second client can deserialize PDX values (e.g. CQ event values) it did not write.
 *
 * <p>Env: GEODE_HOST (127.0.0.1), GEODE_PORT (40404), PROXY_PORT (40499), REGION (helloWorld),
 * KEY (pk).
 */
public final class PdxByIdCapture {

    private PdxByIdCapture() {
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("=== reader got: " + (value == null ? "null" : value.getClass().getName())
                + " -> " + value + " ===");
        Thread.sleep(500);
